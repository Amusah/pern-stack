...
router.get("/cars", async (req, res) => {
  const allCars = await db.select().from(cars); // Fetch all cars from the database
  res.json(allCars);
});
...
router.post("/cars", async (req, res) => {
  ...
  const [newCar] = await db.insert(cars).values({ make, model, year, price }).returning(); // Insert and return the new car
